...
def FCN8(nClasses, input_height, input_width):
    ...
    skip_con1 = Conv2D(filters=nClasses, kernel_size=1, padding="same", activation=None, kernel_initializer="he_normal", name="score_pool4")(fcn8.get_layer("block4_pool").output)
    Summed = add(inputs=[skip_con1, fcn8.output])
    
    x = Conv2DTranspose(filters=nClasses, kernel_size=2, strides=2, padding="valid", activation=None, name="score4")(Summed)
    
    # 提取 VGG16 模型层中的 block3_pool 输出进行卷积
    skip_con2 = Conv2D(filters=nClasses, kernel_size=1, padding="same", activation=None, kernel_initializer="he_normal", name="score_pool3")(fcn8.get_layer("block3_pool").output)
    Summed2 = add(inputs=[skip_con2, x])
    
    Up = Conv2DTranspose(filters=nClasses, kernel_size=8, strides=8, padding="valid", activation=None, name="upsample")(Summed2)
