...
if __name__ == '__main__':
    ...
    images_path = "dataset/images_prepped_test/"
    segs_path = "dataset/annotations_prepped_test/"
    ...
    # 创建模型并加载预训练权重
    model = FCN32 (n_classes, input_height, input_width)  # 有自定义层时，不能直接加载模型
    model.load_weights("output/fcn32_model.h5")

    for i, (imgName, segName) in enumerate(zip(images, segmentations)):

        print(f"{i + 1}/{len(images)} {imgName}")
